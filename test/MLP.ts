...
async function setReward(duration?: number, amount?: BigNumber) {
  ...
  if (amount != undefined) {
    await deepfiToken.connect(owner).approve(mlpVault.address, amount)
...
describe("🌞 MLP Adapter Test", async () => {
  ...
  describe("🌱 Check Vault / Adapter / Rewards", async () => {
      ...
      it("Test 1 : User Wallet Basic Compound", async () => {
        console.log("MLP of owner : ", await stakedGLP.balanceOf(owner.address))
